...
class MY_DB_mysql_driver extends CI_DB_mysql_driver {
	...
	function get_var($sql=null, $x = 0, $y = 0) {
		$CI =& get_instance();
		...
		if ( $sql )
			...
			if ( $query ) {
				if ($query->num_rows() > 0)
				{
				   ...
				   reset($values);
				   $first = $values[key($values)];
				}
			}
		// If there is a value return it else return null
		return (isset($first) && $first!=='') ? $first : null;
	}

	
	function set_user_data() {
		$userid = $this->session->userdata('userid');
		$results = $this->db->query("select * from ".$CI->db->dbprefix."users where userid = '$userid'")->result();
